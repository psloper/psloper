...
export const ASSUMPTIONS = {
  ...
  positionNote: 'Farm positions are planning-database centroids, out by about 1,100 m in the median '
    ...
    + 'hidden verdict on up to 5.6 per cent of them, 49 farms.',
};

/**
 * How far each assumption moves the answer, measured rather than asserted.
 *
 * Generated by tools/measure_sensitivity.mjs against the real data: one
 * assumption changed at a time, everything else held, reported as the change
 * in visible pairings from the baseline. test/national.test.mjs recomputes
 * these against the shipped terrain, so a figure cannot go stale without the
 * suite failing.
 *
 * These replaced adjectives. The panel used to call the tip height "the single
 * biggest lever" and say a taller antenna reached "noticeably further". The
 * first is true among the assumptions and the second understated it: a 50 m
 * lattice adds 38 per cent, against 55 for a 250 m tip. Neither is close to
 * the effect of having terrain at all.
 */
